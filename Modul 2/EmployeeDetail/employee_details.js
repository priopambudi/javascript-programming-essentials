...
function calculateTotalSalaries() {
  const totalSalaries = employees.reduce((prev, curr) => prev + curr.salary, 0);
...
function findBySpecialization(specialization) {
  ...
  if (findEmployee) {
    document.getElementById(
      "employeesDetails"
    ).innerHTML = `<p>${findEmployee.id}: ${findEmployee.name}: ${findEmployee.name} - ${findEmployee.department} - $${findEmployee.salary}</p>`;
  } else {
    document.getElementById("employeesDetails").innerHTML =
